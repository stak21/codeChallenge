...
var getIntersectionNode = function (headA, headB) {
  ...
  let currentNodePTR1 = headA.head;
  let currentNodePTR2 = headB.head;
  let count1 = 0;
  let count2 = 0;
  let finalCount = null;
  ...
  function modifyNode(node) {
    for (var i = 0; i < Math.abs(finalCount); i++) {
      node = node.next;
    }
    return node;
  }
  ...
  while (currentNodePTR1 !== null && currentNodePTR2 !== null) {
    ...
    if (finalCount === null) {
      ...
      currentNodePTR1 = headA.head;
      currentNodePTR2 = headB.head;
      if (finalCount > 0) {
        currentNodePTR1 = modifyNode(currentNodePTR1);
      //    else modifyNode(compare2)
      } else {
        currentNodePTR2 = modifyNode(currentNodePTR2);
      }
    }
